...
module Capture
  ...
  def self.kaice
    ...
    serializer = SimpleXlsx::Serializer.new("开测信息/#{Time.now.strftime('%Y%m%d%H%M%S')}.xlsx") do |doc|
  ...
  def self.kaifu
    ...
    serializer = SimpleXlsx::Serializer.new("开服信息/#{Time.now.strftime('%Y%m%d%H%M%S')}.xlsx") do |doc|
      doc.add_sheet(Time.now.to_s) do |sheet|
